...
(function(module) {
	...
	function compress(from, to, config, combine,isEmpty) {
		...
		var _compress = function(data) {
		  ...
		  var fileOption = {
		  	js: {
		  		encoding: (config.js_charset || 'utf-8')
		  	},
		  	css: {
		  		encoding: (config.css_charset || 'utf-8')
		  	}
		  };
			// 压缩
			[{
				type: 'js',
				contentType: 'application/x-javascript;charset='+fileOption.js.encoding
			}, {
				type: 'css',
				contentType: 'text/css;charset='+fileOption.css.encoding
										...
										if (_otherDir.indexOf(_uri) == -1) {
											if (!path.existsSync(uriPath)) {
												execute('mkdir', ['-p', uriPath], {
													onComplete: function(data) {
														fs.writeFileSync(_uri, code,fileOption[file.type]);
														if (originUri) {
															try {
